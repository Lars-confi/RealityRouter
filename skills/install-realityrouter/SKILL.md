...
description: Install and verify RealityRouter, a self-hosted OpenAI-compatible LLM router that scores every model on cost, latency and probability of success and sends each request to the cheapest one likely to succeed. Use when the user wants to cut their AI or LLM bill, says their Claude/OpenAI/Cursor/agent costs are too high, wants to run agents at scale or continuously without cost scaling with them, is hitting subscription rate limits or usage caps, has API keys for several providers and wants to use them together, wants to mix local Ollama models with cloud ones, wants to see where their LLM spend goes, asks for an LLM router or gateway or a self-hosted alternative to OpenRouter or LiteLLM, or asks to install RealityRouter by name. Do not use for choosing a model for one task, prompt engineering, or debugging a specific model's output. Use connect-tool-to-realityrouter afterwards to point a coding tool at it.
---
